...
def load_pipeline_data(dataset: str, use_ba: bool, use_masks: bool, reproject_depth: bool):
    ...
    return output_dir, images, world_points, depth_conf, masks, extrinsics, intrinsics, pose_source
...
def train(args):
    ...
    output_dir, images, world_points, depth_conf, masks, extrinsics, intrinsics, pose_source = load_pipeline_data(
    ...
    points_np, colors_np, conf_np = sample_initial_points(
        world_points, train_images, depth_conf, foreground_masks, args.max_points, args.conf_threshold, args.seed
    )
    ...
    for step in pbar:
        ...
        if weights is not None:
            ...
            l1 = (l1_loss(renders, gt) * weights3).sum() / weights3.sum().clamp_min(1.0)
            if args.bg_mode == "foreground-only":
                ssim = ssim_loss(
                    (renders * weights3).permute(0, 3, 1, 2),
                    (gt * weights3).permute(0, 3, 1, 2),
                )
            else:
                ssim = ssim_loss(renders.permute(0, 3, 1, 2), gt.permute(0, 3, 1, 2))
        else:
            l1 = l1_loss(renders, gt).mean()
            ssim = ssim_loss(renders.permute(0, 3, 1, 2), gt.permute(0, 3, 1, 2))
        ...
        if step % args.eval_every == 0 or step == steps - 1:
            with torch.no_grad():
                psnr = compute_psnr(renders[0].clamp(0.0, 1.0), gt[0])
...
def main():
    ...
    parser.add_argument("--refine-stop", type=int, default=15000)
